...
namespace DevLifeBackend.Services
{
    ...
    {
        // Update the method signature to accept difficulty
    ...
    public class CodeRoastService : ICodeRoastService
    {
        ...
        {
            // Pass the difficulty to the CodewarsService
        ...
        public async Task<RoastResultDto?> GetRoastAnalysisAsync(string language, string sourceCode)
        {
            ...
            {
                return new RoastResultDto { ExecutionStatus = "Execution Failed", AiRoast = "I couldn't even run your code to roast it. Is it even real?" };
            }

            var executionStatus = executionResult.Status?.Description ?? "Unknown Status";
            var executionOutput = executionResult.Stderr ?? executionResult.CompileOutput ?? executionResult.Stdout;

            // Step 2: Generate a roast from OpenAI based on the result
            var prompt = executionStatus == "Accepted"
                ? $"This code was submitted for a programming challenge and it works correctly. Write a short, witty, and slightly humorous compliment about it. Be a bit sarcastic, like a senior developer reviewing a junior's code. Return only the compliment. The code is:\n\n{sourceCode}"
                : $"This code was submitted for a programming challenge and it failed. The execution status is '{executionStatus}' and the error output is '{executionOutput}'. Write a short, witty, and funny 'roast' of this code. Be sarcastic but not mean. Return only the roast. The code is:\n\n{sourceCode}";
            ...
            try
            {
                ...
                aiRoast = response.FirstChoice.Message.Content ?? aiRoast;
            }
            catch (Exception ex)
            {
